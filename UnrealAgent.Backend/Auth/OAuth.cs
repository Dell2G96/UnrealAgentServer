...
public class OAuth(IHttpClientFactory HttpClientFactory)
{
    // <summary> 마지막 OAuth 플로우에서 발생한 에러메세지
    ...
    // OAuth 요청 스코프
    private const string Scopes = "org:create_api_key user:profile user:interface";

    // Anthropic OAuth 콜백 URL
    ...
    private async Task<OAuthTokenResponse?> ExchangeCodeAsync(string Code, string State, string CodeVerifier,
